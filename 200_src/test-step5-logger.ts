...
class TestDistributionServiceWithLogger {
  ...
  private async getTokenDecimals(tokenAddress: PublicKey): Promise<number> {
    try {
      ...
      this.logger.warn('Could not get token decimals, using default', { tokenAddress: tokenAddress.toString() });
      return 9;
    } catch (error) {
...
async function main() {
  try {
    console.log('🚀 Step 5: Service with Tributary Logger');
    console.log('=======================================');
    ...
    // 2. サービス作成
    const service = new TestDistributionServiceWithLogger('testnet', adminKeypair);
