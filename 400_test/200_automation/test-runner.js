...
class TestRunner {
  constructor() {
    ...
    this.config = {
      ...
      maxRetries: 3,
      testDir: path.join(os.tmpdir(), 'tributary-test-' + Date.now()),
      networks: {
  ...
  async verifyPrerequisites() {
    ...
    for (const check of checks) {
      try {
        const version = await this.execCommand(check.cmd);
  ...
  async testForceOverwrite() {
    const testDir = path.join(this.config.testDir, 'force-overwrite');
    await fs.mkdir(testDir, { recursive: true });

    // Create initial config
    await this.execCommand(
      `cd "${testDir}" && tributary init --name "InitialTest" --token "So11111111111111111111111111111111111111112" --admin "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU" --network devnet`
    );

    // Try to overwrite with --force
    const result = await this.execCommand(
      `cd "${testDir}" && tributary init --name "OverwriteTest" --token "So11111111111111111111111111111111111111112" --admin "7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU" --network devnet --force`
    );
